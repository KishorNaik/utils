...
const service = {
	handleAsync: async (item: { id: number }): Promise<Result<string, any>> => {
		...
		if (item.id % 5 === 0) {
			// Simulate a failure for every 5th item
...
(async () => {
	...
	console.log(
		`Parallel Execution Done. Successes: ${parallelResult.success.length}, Failures: ${parallelResult.error.length}`
	);
	...
	console.log(
		`Sequential Execution Done. Successes: ${sequentialResult.success.length}, Failures: ${sequentialResult.error.length}`
	);
