...
const fetchUser = async (userId: string) => ResultFactory.success({ id: userId, name: 'John Doe', loyaltyPoints: 50 });
const fetchProduct = async (productId: string) => ResultFactory.success({ id: productId, name: 'Widget', stock: 10 });
const chargeCard = async (userId: string, amount: number) => ResultFactory.success({ transactionId: 'txn-abc' });
