...
async function executeCreateOrderSaga() {
	...
	const initialContext: IOrderSagaContext = {
		userId: 'user-abc',
		productId: 'prod-xyz',
		quantity: 2,
	};
	...
	try {
		saga.step({
			...
			.step({
				...
				action: async (ctx) => {
					...
					if (result.isOk())
						ctx.context.paymentTransactionId = result.value.paymentTransactionId;
					return result;
				},
		...
		if (saga.getContext().isSuccess) {
			...
			const paymentResult = saga.getStepResult<{ paymentTransactionId: string }>(
				'ProcessPayment'
			);
